...
Active Tornado Warnings and Flash Flood Warnings for the selected coordinates appear in a translucent red banner pinned to the top of the screen. It pulses slowly (solid with reduced motion enabled). Alerts refresh every minute while the page is visible and when returning to the page, independently of forecast availability. Expired warnings are removed; if updates fail, unexpired warnings remain with an update-unavailable notice. ZIP locations use the coordinates returned by the ZIP lookup.
